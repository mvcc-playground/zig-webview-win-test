...
const command_runtime = @import("command_runtime.zig");
const build_options = @import("build_options");

pub fn run(comptime app_commands: type) !void {
    _ = app_commands;
    ...
    const title = try app_url.dupZ(allocator, build_options.app_title);
    defer allocator.free(title);

    const bind_name = try app_url.dupZ(allocator, build_options.invoke_binding);
    defer allocator.free(bind_name);

    const exit_code = mini_webview_run(url.ptr, title.ptr, bind_name.ptr);
    if (exit_code != 0) {
        return error.WebviewRunFailed;
    }
}

pub fn handleInvoke(comptime app_commands: type, req_json: [*:0]const u8, out_json: [*]u8, out_len: usize) c_int {
    ...
    const result = command_runtime.dispatch(app_commands, allocator, command, args) catch |err| {
        return writeFailure(out_json, out_len, .{
...
fn writeFailure(out_json: [*]u8, out_len: usize, failure: command_runtime.NativeFailure) c_int {
...
extern fn mini_webview_run(start_url: [*:0]const u8, title: [*:0]const u8, bind_name: [*:0]const u8) callconv(.c) c_int;
